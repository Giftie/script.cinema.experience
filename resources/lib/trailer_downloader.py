...
def downloader( mpaa, genre ):
    movie = ""
    trailer_list = []
    utils.log( "%s - Starting Trailer Downloader" % logmessage, level=xbmc.LOGNOTICE )
    trailer_list = _download_trailers( mpaa, genre, movie )
    save_download_list( trailer_list )


def save_download_list( download_trailers ):
    utils.log( "%s - Saving List of Downloaded Trailers" % logmessage, level=xbmc.LOGNOTICE )
    success = False
    try:
    ...
    if not success:
        try:
            utils.log( "%s - Removing List of Downloaded Trailers" % logmessage, level=xbmc.LOGNOTICE )
            if ( os.path.isfile( base_path ) ):
                os.remove( base_path )
        except:
            utils.log( "%s - Error Trying to Remove List of Downloaded Trailers" % logmessage, level=xbmc.LOGNOTICE )

def _download_trailers( mpaa, genre, movie ):
    updated_trailers = []
    utils.log( "%s - Downloading Trailers: %s Trailers" % ( logmessage, ( 0, 1, 2, 3, 4, 5, 10, )[ int( _S_( "trailer_count" ) ) ] ), level=xbmc.LOGNOTICE )
    ...
    for trailer in trailers:
        updated_trailer= {}
        utils.log( "%s - Attempting To Download Trailer: %s" % ( logmessage, trailer[ 1 ] ), level=xbmc.LOGNOTICE )
        ...
        if success:
            utils.log( "%s - Successfully Download Trailer: %s" % ( logmessage, trailer[ 1 ] ), level=xbmc.LOGNOTICE )
            ...
            utils.log( "%s - Failed to Download Trailer: %s" % ( logmessage, trailer[ 1 ] ), level=xbmc.LOGNOTICE )
            updated_trailer=[]
...
def _create_nfo_file( trailer, trailer_nfopath ):
    '''
            ...
            director=trailer[ 11 ]
    '''
    utils.log( "%s - Creating Trailer NFO file" % logmessage, level=xbmc.LOGNOTICE )
...
def _save_nfo_file( nfoSource, trailer_nfopath ):
    utils.log( "%s - Saving Trailer NFO file" % logmessage, level=xbmc.LOGNOTICE )
    ...
    try:
        ...
        utils.log( "%s - %s" % ( logmessage, str( e ) ), xbmc.LOGERROR )
        # return failed
        return False
