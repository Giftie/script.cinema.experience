...
def downloader( mpaa, genre, equivalent_mpaa ):
    movie = ""
    trailer_list = []
    log( "Starting Trailer Downloader", level=xbmc.LOGNOTICE )
...
def save_download_list( download_trailers ):
    log( "Saving List of Downloaded Trailers", level=xbmc.LOGNOTICE )
    success = False
    try:
    ...
    if not success:
        try:
            log( "Removing List of Downloaded Trailers", level=xbmc.LOGNOTICE )
            if ( os.path.isfile( base_path ) ):
                os.remove( base_path )
        except:
            log( "Error Trying to Remove List of Downloaded Trailers", level=xbmc.LOGNOTICE )
    
def _download_trailers( equivalent_mpaa, mpaa, genre, movie ):
    updated_trailers = []
    log( "Downloading Trailers: %s Trailers" % trailer_settings[ "trailer_count" ], level=xbmc.LOGNOTICE )
    ...
    for trailer in trailers:
        ...
        destination = ""   
        thumb = ""
        log( "Attempting To Download Trailer: %s" % trailer[ 1 ], level=xbmc.LOGNOTICE )
        ...
        if success:
            log( "Successfully Download Trailer: %s" % trailer[ 1 ], level=xbmc.LOGNOTICE )
            ...
            log( "Failed to Download Trailer: %s" % ( logmessage, trailer[ 1 ] ), level=xbmc.LOGNOTICE )
            updated_trailer=[]
...
def _create_nfo_file( trailer, trailer_nfopath ):
    '''
                ...
                director=trailer[ 11 ]
    '''
    log( "Creating Trailer NFO file", level=xbmc.LOGNOTICE )
...
def _save_nfo_file( nfoSource, trailer_nfopath ):
    log( "Saving Trailer NFO file", level=xbmc.LOGNOTICE )
...
if __name__ == "__main__":
    try:
        if sys.argv[1]:
            ...
            log( "No Arguments sent ", level=xbmc.LOGNOTICE )
    except:
        traceback.print_exc()
        log( "No Arguments sent ", level=xbmc.LOGNOTICE )
