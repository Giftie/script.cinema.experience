...
class Main:
    utils.log("%s - Local Folder Trailer Scraper Started" % logmessage, level=xbmc.LOGNOTICE )
    ...
    def fetch_trailers( self ):
        utils.log("%s - Fetching Trailers" % logmessage, level=xbmc.LOGNOTICE )
        # get watched list
        self._get_watched()
    ...
    def _shuffle_trailers( self ):
        ...
        utils.log("%s - Shuffling Trailers" % logmessage, level=xbmc.LOGNOTICE )
    ...
    def _get_watched( self ):
        utils.log("%s - Getting Watched List" % logmessage, level=xbmc.LOGNOTICE )
        try:
    ...
    def _reset_watched( self ):
        utils.log("%s - Resetting Watched List" % logmessage, level=xbmc.LOGNOTICE )
        if int( self.settings[ "trailer_play_mode" ] )== 1:
            base_path = os.path.join( BASE_CURRENT_SOURCE_PATH, "downloader" + "_watched.txt" )
        else:
            base_path = os.path.join( BASE_CURRENT_SOURCE_PATH, self.settings[ "trailer_scraper" ] + "_watched.txt" )
    ...
    def _save_watched( self ):
        utils.log("%s - Saving Watched List" % logmessage, level=xbmc.LOGNOTICE )
        try:
            # base path to watched file
            if int( self.settings[ "trailer_play_mode" ] )== 1:
                base_path = os.path.join( BASE_CURRENT_SOURCE_PATH, "downloader" + "_watched.txt" )
