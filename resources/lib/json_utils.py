...
def retrieve_json_dict(json_query, items='items', force_log=False ):
    ...
    try:
        if response.has_key( 'result' ):
            ...
            utils.log( "[json_utils.py] - retrieve_json_dict - Error trying to get json response", level=xbmc.LOGDEBUG )
            utils.log( "%s" % response, level=xbmc.LOGDEBUG )
            return None
        else:
            utils.log( "[json_utils.py] - retrieve_json_dict - No response from XBMC", level=xbmc.LOGNOTICE )
            utils.log( "%s" % response, level=xbmc.LOGDEBUG )
            return None
    except:
        traceback.print_exc()
...
def retrieve_movie_db():
    ...
    if movie_db:
        utils.log( "[json_utils.py] - retrieve_movie_database - Successfully retrieved database", level=xbmc.LOGDEBUG )
    else:
        utils.log( "[json_utils.py] - retrieve_movie_database - No database found", level=xbmc.LOGDEBUG )
    return movie_db
...
def find_movie_details( movie_db=None, field="title", match_value=None ):
    ...
    """
    utils.log( "[json_utils.py] - find_movie_details - field: %s, matche_value: %s" % ( field, match_value ), level=xbmc.LOGDEBUG )
    movie_return = None
    if not movie_db:
    ...
    if movie_db:
        ...
        utils.log( "[json_utils.py] - find_movie_details - No database found", level=xbmc.LOGDEBUG )
    return movie_return
...
def insert_movie_into_playlist( movie_db=None, title="", index=0 ):
    ...
    if matched_movie:
        utils.log( "[json_utils.py] - insert_movie_into_playlist - Matched Title: %s" % title, level=xbmc.LOGDEBUG )
        ...
        if result.startswith( "{" ):
            result = eval( result )
        if result['result'] == "OK":
            utils.log( "[json_utils.py] - insert_movie_into_playlist - Title: %s, Succeeded" % title, level=xbmc.LOGDEBUG )
        else:
            utils.log( "[json_utils.py] - insert_movie_into_playlist - Title: %s, Failed" % title, level=xbmc.LOGDEBUG )
    else:
        utils.log( "[json_utils.py] - insert_movie_into_playlist - Unable to match Title: %s" % title, level=xbmc.LOGDEBUG )
...
def add_movie_to_playlist( movie_db=None, title="" ):
    ...
    matched_movie = find_movie_details( movie_db=movie_db, field='title', match_value=title )
    if matched_movie:
        utils.log( "[json_utils.py] - add_movie_to_playlist - Matched Title: %s" % title, level=xbmc.LOGDEBUG )
        ...
        if result.startswith( "{" ):
            result = eval( result )
        if result['result'] == "OK":
            utils.log( "[json_utils.py] - add_movie_to_playlist - Title: %s, Succeeded" % title, level=xbmc.LOGDEBUG )
        else:
            utils.log( "[json_utils.py] - add_movie_to_playlist - Title: %s, Failed" % title, level=xbmc.LOGDEBUG )
    else:
        utils.log( "[json_utils.py] - add_movie_to_playlist - Unable to match Title: %s" % title, level=xbmc.LOGDEBUG )
